...
function renderPrices(){
  $('pricesTable').innerHTML=filteredPrices().map(p=>`<tr><td>${esc(p.brand)}</td><td>${esc(p.model)}</td><td>${esc(p.category)}</td><td>${esc(p.service)}</td><td>${money(p.price)}</td><td><span class="status-pill ${p.is_visible?'ready':'wait'}">${p.is_visible?'Aktywna':'Ukryta'}</span></td><td class="action-cell"><button type="button" class="table-action" data-edit-price="${p.id}">Edytuj</button> <button type="button" class="table-action danger-text" data-delete-price="${p.id}">Usuń</button></td></tr>`).join('')||'<tr><td colspan="7" class="empty-row">Brak pozycji cennika.</td></tr>'
}
function openPriceForm(price=null){
  ...
  $('priceFormTitle').textContent=price?'Edytuj pozycję':'Dodaj pozycję';
  $('priceSubmitButton').textContent=price?'Zapisz zmiany':'Zapisz pozycję';
  $('priceBrand').value=price?.brand||'';
  $('priceModel').value=price?.model||'';
  $('priceCategory').value=price?.category||'';
  $('priceService').value=price?.service||'';
  $('priceAmount').value=price?Number(price.price||0):'';
  $('priceVisible').checked=price?Boolean(price.is_visible):true;
  $('priceDialog').showModal();
}
async function deletePriceItem(id){
  if(guardDemo())return;
  const price=prices.find(x=>String(x.id)===String(id));
  if(!price)return;
  if(!confirm(`Usunąć pozycję „${price.brand} ${price.model} — ${price.service}”?`))return;
  const {error}=await db.from('price_items').delete().eq('id',id).eq('organization_id',currentOrg.id);
  if(error)return alert('Nie udało się usunąć pozycji: '+error.message);
  await loadAll();
}
...
async function openOrderDetails(id){
  ...
  $('detailsDevice').textContent=`${order.brand} ${order.model}`;$('detailsImei').textContent=order.imei?`IMEI/SN: ${order.imei}`:'Brak IMEI / numeru seryjnego';$('detailsUnlockCode').textContent=order.unlock_code||'—';
    ...
    <section class="print-page service-copy">
      <div class="device-wrap-card">
        ...
        <div class="device-main">
          ...
          <p class="device-imei">KOD BLOKADY / PIN: <b>${esc(order.unlock_code||'—')}</b></p>
        </div>
...
$('priceForm').onsubmit=async e=>{
  ...
  const p={organization_id:currentOrg.id,brand:$('priceBrand').value.trim(),model:$('priceModel').value.trim(),category:$('priceCategory').value.trim(),service:$('priceService').value.trim(),price:Number($('priceAmount').value),is_visible:$('priceVisible').checked};
  const query=id
    ?db.from('price_items').update(p).eq('id',id).eq('organization_id',currentOrg.id)
    :db.from('price_items').insert(p);
  const {error}=await query;
  if(error)return $('priceError').textContent=error.message;
  $('priceDialog').close();
  e.target.reset();
  await loadAll();
}
...
$('orderForm').onsubmit=async e=>{
  ...
  try{
    ...
    const p={
      ...
      unlock_code:$('orderUnlockCode').value.trim()||null,
...
$('newPriceButton').onclick=()=>openPriceForm();$('pricesTable').onclick=e=>{const editId=e.target.dataset.editPrice,deleteId=e.target.dataset.deletePrice;if(editId)openPriceForm(prices.find(x=>String(x.id)===String(editId)));if(deleteId)deletePriceItem(deleteId)};$('newInventoryButton').onclick=()=>openInventoryForm();$('importInvoiceButton').onclick=()=>{if(guardDemo())return;invoiceLines=[];renderInvoiceLines();$('invoiceFile').value='';$('invoiceStatus').textContent='';$('invoiceNumber').value='';$('invoiceSupplier').value='';$('invoiceDate').value=new Date().toISOString().slice(0,10);$('invoiceDialog').showModal()};
